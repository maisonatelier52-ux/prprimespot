...
// This renders automatically for:
//  - any URL that matches no route in the app
//  - every explicit notFound() call (unknown category / unknown article
//    slug in app/[category]/page.jsx and app/[category]/[slug]/page.jsx)
// Root layout still wraps this with <Header /> / <Footer />, so we only
// need the page body here.
    ...
    <main className="w-full max-w-[100vw] overflow-x-hidden bg-white text-[#1A1A1A]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="flex justify-center mb-12">
          <Link
            href="/"
            className="inline-block bg-[#D01418] px-8 py-3 font-sans text-sm font-semibold uppercase tracking-wide text-white hover:bg-[#a80f12] transition-colors"
          >
            Back to Homepage
          </Link>
        </div>
        ...
        <div className="border-t border-[#E5E5E5] pt-8">
          <h2 className="font-serif text-base font-bold text-[#1A1A1A] text-center mb-5">
            Or browse a section
          </h2>
          <div className="flex flex-wrap justify-center gap-3">
            {CATEGORY_LINKS.map((cat) => (
              <Link
                key={cat.href}
                href={cat.href}
                className="border border-[#E5E5E5] bg-white px-5 py-2 font-sans text-sm font-semibold text-[#1A1A1A] hover:border-[#D01418] hover:text-[#D01418] transition-colors"
              >
                {cat.label}
              </Link>
            ))}
